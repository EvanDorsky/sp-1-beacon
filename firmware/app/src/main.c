...
static int usb_present(void) {
  return nrf_gpio_pin_read(SP1_CHG_NPGOOD) == 0;
}
static int charging(void) {
  return nrf_gpio_pin_read(SP1_CHG_NCHG) == 0;
}
...
static void power_off(void) {
  ...
  nrf_gpio_cfg_sense_input(SP1_FUNC_BTN, NRF_GPIO_PIN_PULLUP, NRF_GPIO_PIN_SENSE_LOW);
...
#define POLL_ON_MS                                                                                 \
  8 /* active scan cadence (rail held on). The button                                              \
...
#define SEND_RETRY_MS                                                                              \
  50 /* before the module acks, re-send this fast so a                                             \
      * cold-boot-race SET_STATE isn't lost for ~1 s */
#define EVENT_DWELL_MS                                                                             \
  600 /* keep an event packet on the air at least this                                             \
...
#define FADER_MIN_MS                                                                               \
  25                        /* min gap between fader packets while sliding —                       \
...
static void payload_send(int64_t now) {
  ...
  int n = (cur_kind == PKT_FADERS) ? bthome_encode_faders(pid, battery, cur_fader, p, sizeof(p))
